...
import { FileServerHtmlBundleHandler } from './file/FileServerHtmlBundleHandler.js';
...
export class FileServerService {
	...
	private htmlBundleHandler: FileServerHtmlBundleHandler;
	...
	constructor(
		...
		this.htmlBundleHandler = new FileServerHtmlBundleHandler(
			this.config,
			this.driveFilesRepository,
			this.internalStorageService,
		);
	...
	public createServer(fastify: FastifyInstance, options: FastifyPluginOptions, done: (err?: Error) => void) {
		...
		fastify.register((fastify, options, done) => {
			...
			// HTML Bundle file serving
			fastify.get<{ Params: { fileId: string; '*': string } }>('/files/:fileId/html-bundle/*', async (request, reply) => {
				// Extract path from wildcard
				const filePath = (request.params as any)['*'] || '';
				return await this.htmlBundleHandler.handle(
					{ ...request, params: { fileId: request.params.fileId, path: filePath } } as any,
					reply,
				).catch(err => this.errorHandler(request, reply, err));
			});
			done();
