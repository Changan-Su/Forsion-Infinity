	...
	<div v-if="mediaList.filter(media => previewable(media)).length > 0" :class="$style.container">
		...
		>
			<template v-for="media in mediaList.filter(media => previewable(media))">
				...
				<XHtmlBundle v-else-if="media.isHtmlBundle" :key="`html-bundle:${media.id}`" :class="$style.media" :media="media"/>
			</template>
		</div>
	</div>
</div>
...
import XHtmlBundle from '@/components/MkMediaHtmlBundle.vue';
...
const previewable = (file: Misskey.entities.DriveFile): boolean => {
	...
	if (file.isHtmlBundle) return true; // HTML Bundle is previewable
